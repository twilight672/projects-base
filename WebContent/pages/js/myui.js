...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            impor: function(id) {
               // alert("bbb");
                var url = $.common.isEmpty(id) ? $.table._option.imporUrl : $.table._option.imporUrl.replace("{id}", id);
            ...
            editTable: function(tableName) {
                var url = "/404.html";
                if ($.common.isNotEmpty(tableName)) {
                    url = $.table._option.updateUrl.replace("{tableName}", tableName);
                } else {
                    var tableName = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
            ...
            saveAndUpload: function(url, data) {
            	$.modal.loading("正在处理中，请稍后...");
            	var config = {
        	        url: url,
        	        type: "post",
