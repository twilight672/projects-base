...
async function main() {
  ...
  if (!address || typeof address === "string") throw new Error("Cannot resolve smoke server address");
  ...
  const createdJson = (await created.json()) as { code: string };
  const queried = await fetch(`${base}/api/complaints/${createdJson.code}`);
  if (!queried.ok) throw new Error(`query failed: ${queried.status}`);
