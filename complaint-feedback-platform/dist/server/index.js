...
function adminComplaint(row, req) {
...
function countyWhere(req) {
    if (req.user?.role === "county")
        return { sql: " AND county = ?", params: [req.user.county] };
    return { sql: "", params: [] };
}
...
app.post("/api/complaints", upload.array("attachments", 5), (req, res) => {
    ...
    run(`INSERT INTO complaints
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [code, mode, county, category, title, content, name || null, phone || null, "待受理", null, timestamp, timestamp, 0]);
    ...
    for (const file of files)
        saveAttachment(complaint.id, "initial", file);
...
app.post("/api/complaints/:code/supplement", upload.array("attachments", 5), (req, res) => {
    ...
    for (const file of files)
        saveAttachment(complaint.id, "supplement", file);
...
app.get("/api/admin/stats", requireAuth, (req, res) => {
    ...
    const rows = all(`SELECT status, COUNT(*) as total FROM complaints WHERE 1=1${where.sql} GROUP BY status`, where.params);
    const countyRows = all(`SELECT county, COUNT(*) as total FROM complaints WHERE 1=1${where.sql} GROUP BY county ORDER BY total DESC`, where.params);
    const total = get(`SELECT COUNT(*) as total FROM complaints WHERE 1=1${where.sql}`, where.params)?.total ?? 0;
...
app.post("/api/admin/complaints/:id/actions", requireAuth, (req, res) => {
    const complaint = findComplaintById(Number(req.params.id));
    if (!complaint || (req.user?.role === "county" && complaint.county !== req.user.county)) {
        res.status(404).json({ message: "未查询到对应事项" });
        return;
...
app.get("/api/admin/export.csv", requireAuth, (req, res) => {
    const where = countyWhere(req);
    const rows = all(`SELECT * FROM complaints WHERE 1=1${where.sql} ORDER BY id DESC`, where.params);
    const header = ["编号", "县区", "类型", "标题", "提交方式", "状态", "承办单位", "创建时间", "更新时间"];
    const body = rows.map((row) => [row.code, row.county, row.category, row.title, row.mode === "realname" ? "实名" : "匿名", row.status, row.assignee || "", row.created_at, row.updated_at]
